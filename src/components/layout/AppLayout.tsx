  ...
  return (
    <div className="h-screen flex bg-slate-100 dark:bg-slate-950 font-sans text-slate-900 dark:text-slate-100 antialiased overflow-hidden transition-colors duration-200">
        ...
        <div className={cn('border-t border-slate-800/60 flex-shrink-0', collapsed ? 'p-2' : 'p-3')}>
          ...
          <div className={cn('flex', collapsed ? 'flex-col gap-1.5 items-center' : 'gap-1.5')}>
            ...
            >
              ...
              {!collapsed && <span className="text-[11px] font-bold">{theme === 'light' ? 'Dark' : 'Light'}</span>}
              {collapsed && <span className="sidebar-tooltip">{theme === 'light' ? 'Dark Mode' : 'Light Mode'}</span>}
            </button>
            <button
      ...
      <div className="flex-1 flex flex-col min-h-0 overflow-hidden relative">
        ...
        {/* TOP BAR — mobile */}
        <header className="md:hidden flex items-center justify-between h-14 px-4 bg-white dark:bg-slate-900 border-b border-slate-200/70 dark:border-slate-800/80 shadow-sm flex-shrink-0 z-30 transition-colors duration-200">
            ...
            <div className="flex flex-col justify-center">
              <div className="flex items-center gap-1.5">
                <span className="text-base font-black tracking-tight leading-none">
                  ...
                  <span className="text-slate-800 dark:text-white"> Mess</span>
                </span>
          ...
          <div className="flex items-center gap-1.5">
            <input
              ...
              value={selectedMonth}
              onChange={(e) => setSelectedMonth(e.target.value)}
              className="bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-2 py-1 text-xs font-bold text-slate-700 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500/30 cursor-pointer"
            />
            <NotificationCenter />
          </div>
        </header>
        ...
        <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50">
          <div className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border-t border-slate-200/80 dark:border-slate-800/80 shadow-2xl shadow-slate-900/10">
                ...
                return (
                  ...
                  >
                    <div className={cn(
                      ...
                      <item.icon className={cn('w-5 h-5 transition-colors', isActive ? 'text-white' : 'text-slate-400 dark:text-slate-500')} />
                    </div>
                    <span className={cn(
          ...
          {moreOpen && (
            <>
              ...
              <motion.div
                ...
                className="md:hidden fixed inset-x-0 bottom-0 z-[70] bg-white dark:bg-slate-900 rounded-t-3xl overflow-hidden shadow-2xl border-t border-slate-200/50 dark:border-slate-800/80 transition-colors duration-200"
              >
                {/* Drag handle */}
                ...
                <div className="flex items-center justify-between px-5 py-3 border-b border-slate-100 dark:border-slate-800">
                  <div>
                    <h3 className="font-bold text-slate-800 dark:text-white text-base">{language === 'bn' ? 'সব মেনু' : 'All Sections'}</h3>
                    <p className="text-[11px] text-slate-400 font-medium mt-0.5">{language === 'bn' ? 'যেকোনো পেজে যান' : 'Navigate anywhere'}</p>
                  </div>
                  <button
                    onClick={() => setMoreOpen(false)}
                    className="w-8 h-8 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 flex items-center justify-center text-slate-500 dark:text-slate-400 transition-colors"
                  >
                    <X className="w-4 h-4" />
                  </button>
                </div>
                    ...
                    return (
                      ...
                      >
                        <div className={cn(
                          ...
                          isActive ? 'bg-white/20' : 'bg-white dark:bg-slate-800 shadow-sm border border-slate-100/50 dark:border-slate-700/80'
                        )}>
                          <item.icon className={cn('w-4.5 h-4.5', isActive ? 'text-white' : 'text-slate-500')} />
                        </div>
                        <span className={cn('text-sm font-semibold leading-tight truncate', isActive ? 'text-white' : 'text-slate-700 dark:text-slate-300')}>
                          {item.name}
                        </span>
                      </NavLink>
                    );
                ...
                {/* Bottom actions */}
                <div className="px-4 pb-6 grid grid-cols-3 gap-2 border-t border-slate-100 dark:border-slate-800 pt-3">
                  <button
                    onClick={() => setLanguage(language === 'en' ? 'bn' : 'en')}
                    className="flex items-center justify-center gap-1.5 p-3 bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-750 rounded-2xl transition-colors"
                  >
                    <Globe className="w-4 h-4 text-slate-500" />
                    <span className="text-xs font-semibold text-slate-700 dark:text-slate-300">{language === 'en' ? 'বাং' : 'EN'}</span>
                  </button>
                  <button
                    onClick={toggleTheme}
                    className="flex items-center justify-center gap-1.5 p-3 bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-750 rounded-2xl transition-colors"
                  >
                    ...
                    <span className="text-xs font-semibold text-slate-700 dark:text-slate-300">{theme === 'light' ? 'Dark' : 'Light'}</span>
                  </button>
                  <button
                    onClick={() => { handleLogout(); setMoreOpen(false); }}
                    className="flex items-center justify-center gap-1.5 p-3 bg-red-50 dark:bg-red-950/20 hover:bg-red-100 dark:hover:bg-red-950/40 rounded-2xl transition-colors"
                  >
                    <LogOut className="w-4 h-4 text-red-500" />
                    <span className="text-xs font-semibold text-red-600 dark:text-red-400">{t('nav.sign_out')}</span>
                  </button>
                </div>
              </motion.div>
            </>
